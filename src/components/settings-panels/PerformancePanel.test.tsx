...
describe("PerformancePanel", () => {
  ...
  it("has aria-expanded attribute on header", () => {
    ...
    const button = screen.getByRole("button", { name: /Performance & Quality/i });
  ...
  it("changes video codec", () => {
    ...
    expect(defaultProps.onSettingsChange).toHaveBeenCalledWith({ videoCodec: "h265" });
  });

  it("changes video encoder", () => {
    render(<PerformancePanel {...defaultProps} />);
    const input = screen.getByPlaceholderText("Default (auto-select)");
    fireEvent.change(input, { target: { value: "OMX.qcom.video.encoder.avc" } });
    expect(defaultProps.onSettingsChange).toHaveBeenCalledWith({ videoEncoder: "OMX.qcom.video.encoder.avc" });
  });
  ...
  it("changes video buffer", () => {
    ...
    expect(defaultProps.onSettingsChange).toHaveBeenCalledWith({ videoBuffer: 50 });
  });

  it("toggles power off on close", () => {
    render(<PerformancePanel {...defaultProps} />);
    const checkbox = screen.getByText("Power Off on Close").closest("label")!.querySelector("input")!;
    fireEvent.click(checkbox);
    expect(defaultProps.onSettingsChange).toHaveBeenCalledWith({ powerOffOnClose: true });
  });

  it("toggles no power on", () => {
    render(<PerformancePanel {...defaultProps} />);
    const checkbox = screen.getByText("Don't Power On").closest("label")!.querySelector("input")!;
    fireEvent.click(checkbox);
    expect(defaultProps.onSettingsChange).toHaveBeenCalledWith({ noPowerOn: true });
  });
});
